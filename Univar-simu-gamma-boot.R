#single biomarker simualtion(gamma settings)
library(mgcv)
...
for(j1 in 1:length(senseq)){
  ...
  for(j2 in 1:length(nseq)){
    ...
    save(res.l,file=paste0("n",n,"_RUN",RUN,"_se",senseq[j1],"_gammaBoot.Rdata"))
...
cover.allcase 
#write_xlsx(perf.allcase,"perf-allcase-gamma.xlsx")
write_xlsx(cover.allcase,"cover-allcase-gamma.xlsx")
